    ...
    , public CefDisplayHandler
{
  ...
  public:
    renderer_client(spl::shared_ptr<core::frame_factory>       frame_factory,
  ...
  private:
    ...
    {
        not_found_ = true;
        CASPAR_LOG(warning) << print() << L" " << errorText.ToWString() << L" while loading url: \""
                            << failedUrl.ToWString() << L"\"";
    ...
    {
        CASPAR_LOG(info) << print() << L" OnAudioStreamError: \"" << message.ToWString() << L"\"";
    ...
    {
        return name_ + L"[" + url_ + L"]" + L" " + std::to_wstring(format_desc_.square_width) + L" " +
...
class renderer_producer : public core::frame_producer
{
    ...
    CefRefPtr<renderer_client> client_;
  ...
  public:
        ...
        , name_(std::move(name))
    {
        invoke([&] {
            ...
            CefBrowserHost::CreateBrowser(window_info, client_.get(), url_, browser_settings, nullptr, nullptr);
        });
    }

    ~renderer_producer() override
    {
        if (client_ != nullptr)
            client_->close();
    }
